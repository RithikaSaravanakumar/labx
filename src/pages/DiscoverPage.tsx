              ...
              {projects.map(project => (
                <motion.div key={project.id} variants={staggerItem}>
                  <Link to={`/projects/${project.id}`} className="labx-card p-6 block h-full group">
                    ...
                    <h3 className="text-lg font-bold text-labx-text mb-2 group-hover:text-labx-violet transition-colors">{project.name}</h3>
                    ...
                    <div className="mb-4">
                      ...
                      <div className="h-1.5 bg-labx-bg rounded-full overflow-hidden">
                        <div className="h-full rounded-full bg-gradient-to-r from-labx-violet to-labx-cyan" style={{ width: `${project.progress}%` }} />
                      </div>
                    </div>
              ...
              {startups.map(startup => (
                <motion.div key={startup.id} variants={staggerItem}>
                  <Link to={`/startups/${startup.id}`} className="labx-card p-6 block h-full group">
                    ...
                    <h3 className="text-lg font-bold text-labx-text mb-2 group-hover:text-labx-violet transition-colors">{startup.name}</h3>
                  ...
                  <Link to={`/hackathons/${hackathon.id}`} className="labx-card p-6 block group">
                    <div className="flex items-center gap-2 mb-3">
                      <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider ${
                        ...
                        hackathon.status === 'upcoming' ? 'bg-labx-violet/20 text-labx-violet' :
                    ...
                    <h3 className="text-lg font-bold text-labx-text mb-1 group-hover:text-labx-violet transition-colors">{hackathon.name}</h3>
            ...
            <motion.div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6" variants={staggerContainer} initial="hidden" animate="visible">
              {opportunities.map(opp => (
                <motion.div key={opp.id} variants={staggerItem} className="labx-card p-6">
                  <div className="flex items-center gap-2 mb-3">
                    <span className="px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider bg-labx-cyan/10 text-labx-cyan">{opp.type}</span>
