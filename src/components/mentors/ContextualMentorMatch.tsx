...
export default function ContextualMentorMatch({ mentor }: ContextualMentorMatchProps) {
  ...
  return (
    ...
    >
      <div className="absolute top-0 right-0 bg-gradient-to-l from-labx-violet/20 to-transparent text-labx-violet text-[10px] font-mono font-bold px-3 py-1 rounded-bl-xl border-b border-l border-labx-violet/30">
        ⚡ {matchScore}% AI Match
      </div>
      ...
      <div>
        <div className="flex items-center gap-4 mb-4">
          <img
            src={mentor.avatar}
            alt={mentor.name}
            className="w-14 h-14 rounded-2xl border-2 border-labx-violet/50 object-cover group-hover:border-labx-violet transition-colors"
          />
          <div>
            <h3 className="text-base font-bold text-labx-text group-hover:text-labx-violet transition-colors flex items-center gap-1.5">
              {mentor.name}
        ...
        <div className="mb-4 bg-labx-surface/60 border border-labx-border/60 p-2.5 rounded-xl">
          <div className="text-[10px] uppercase font-mono font-bold text-labx-violet mb-1">Contextual Synergy</div>
