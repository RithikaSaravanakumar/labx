    ...
    <div className="labx-card p-6">
      <div className="flex items-center justify-between mb-6">
        <h3 className="text-xl font-bold text-labx-text flex items-center gap-2">
          <Zap className="w-5 h-5 text-labx-cyan" />
          Innovation Roadmap
        </h3>
        <span className="text-xs font-mono font-bold text-labx-cyan bg-labx-cyan/10 px-3 py-1 rounded-full border border-labx-cyan/20">
      ...
      <div className="relative">
        ...
        <div 
          className="absolute left-[19px] top-4 w-0.5 bg-gradient-to-b from-labx-cyan to-cyan-300"
              ...
              >
                ...
                <div className="flex-1 pt-1 min-w-0">
                  <div className="flex items-center justify-between gap-2 mb-1">
                    <h4 className={`font-bold uppercase tracking-wider ${isActive ? 'text-labx-cyan' : 'text-labx-text'}`}>
                      {stage.name}
                    </h4>
                    {stage.rewardPoints > 0 && (
                      <span className={`text-[10px] font-mono font-bold px-2 py-0.5 rounded ${
                        isCompleted ? 'bg-labx-cyan/10 text-labx-cyan' : 'bg-labx-surface border border-labx-border text-labx-text-muted'
                      }`}>
                ...
                {isActive && (
                  <div className="absolute right-4 top-1/2 -translate-y-1/2 opacity-0 md:opacity-100">
                    <ChevronRight className="w-5 h-5 text-labx-cyan" />
                  </div>
                )}
              </motion.button>
            );
