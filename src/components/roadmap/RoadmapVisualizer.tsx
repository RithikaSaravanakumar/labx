            ...
            >
              ...
              {!isLast && (
                <div 
                  ...
                  } ${isCompleted ? 'bg-[#00FF87]' : 'bg-white/10'}`} 
                />
              )}
              ...
              <div className={`relative flex ${isVertical ? 'flex-row items-start gap-4' : 'flex-col items-center'}`}>
                ...
                <div className={`
                  ...
                  ${isCompleted ? 'bg-[#00FF87] border-[#00FF87] text-black shadow-[0_0_15px_rgba(0,255,135,0.4)]' : ''}
                  ${isCurrent ? 'bg-[#0A0C0B] border-[#00FF87] text-[#00FF87] shadow-[0_0_20px_rgba(0,255,135,0.6)]' : ''}
                ...
                `}>
                  ...
                  {isCurrent && (
                    <motion.div
                      ...
                      className="w-2.5 h-2.5 bg-[#00FF87] rounded-full"
                    />
                  )}
                ...
                `}>
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    {isCurrent && (
                      <span className="px-1.5 py-0.5 rounded text-[9px] font-bold bg-emerald-500/20 text-[#00FF87] uppercase whitespace-nowrap">
                        You Are Here
                      </span>
                    )}
                  </div>
                  ...
                  {showDetails && (
                    <>
                      ...
                      {isCurrent && (
                        <div className="mt-3 bg-white/5 rounded-xl p-3 border border-emerald-500/20">
                          <div className="flex justify-between text-[10px] font-bold text-zinc-400 mb-1.5 uppercase">
                            <span>Stage Progress</span>
                            <span className="text-[#00FF87]">{stage.progress}%</span>
                          </div>
                          ...
                          {stage.milestones.find(m => m.status === 'AVAILABLE' || m.status === 'IN_PROGRESS') && (
                            <div className="mt-3 flex items-start gap-2">
                              <ChevronRight className="w-3 h-3 text-[#00FF87] shrink-0 mt-0.5" />
                              <div>
